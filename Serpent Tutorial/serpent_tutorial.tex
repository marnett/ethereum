...
\begin{center}
	...
	wei & $1.0 x 10^18$ \\ \hline
	Kwei & $1.0 x 10^15$ \\ \hline
	Mwei & $1.0 x 10^12$ \\ \hline
	Gwei & $1.0 x 10^9$ \\ \hline
	Szabo & $1.0 x 10^6$ \\ \hline
	Finney & $1000$ \\ \hline
	Ether & $1$ \\ \hline
	Kether & $.001$ \\ \hline
	Mether & $1.0x10^-6$ \\ \hline
	Gether & $1.0x10^-9$ \\ \hline
	Tether & $1.0x10^-12$ \\ \hline
	\end{tabular}
\end{center}
...
When using the tester, we can simply send an arbitrary amount of gas (that is above the amount the contract needs to execute) since it is free. However, when executing contracts on an actual block chain, we need to make sure that we only spend what we need to. ****HOW DO WE CALCULATE GAS NEEDED*** - we could do log tx.gas beginning end

Now that we know the quantity of gas needed to execute the transaction, we need to figure out how much that will cost. Currently, on the public block chain, gas cost 10 szabo per unit. However, that unit will adjust when ethereum is officially released. Szabo is a unit of ether, equal to 1000000000000 wei. The total price of the contract will be equal to the gas price multiplied by the gas cost of the transaction.
