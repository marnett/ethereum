...
As shown in Figure~\ref{fig:schematic},%\elaine{refer}, 
...
to illustrate smart contract programming.

\elaine{talk about wei and ether}
...
def receiveSolution(solution):
	if(self.isSolved == 0 AND SHA3([self.puzzle, solution],2) < self.target):
...
%\subfigure[a]{
\begin{mdframed}
\begin{minted}[breaklines, tabsize=2, fontsize=\footnotesize, linenos, xleftmargin=10pt, firstnumber=34]{python}
def finalize():
...
\subsection{Even More Subtle Errors.}
...
Figure~\ref{fig:incentive}, a party can 
