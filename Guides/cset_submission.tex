...
def add_player(choice):
	if num_players < 2 and msg.value == 1000:
		...
		player[self.num_players].address = msg.sender
		player[self.num_players].choice = choice
...
def add_player(player_commitment):
	if num_players < 2 and msg.value >= 1000:
		reward = reward + msg.value
		player[self.num_players].address = msg.sender
		player[self.num_players].commit = player_commitment
		num_players = num_players + 1
		if msg.value - 1000 > 0:
...
def open(choice, nonce):
	...
	if sha3([msg.sender, choice, nonce], items=3) == self.player[player_num].commit and not player[player_num].has_revealed:
